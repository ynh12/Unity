    ...
    void applyMatcap(inout float4 finalColor, inout float3 matcapEmission)
    {
        ...
        half2 matcapUV = half2(dot(worldViewRight, poiMesh.normals[float(1)]), dot(worldViewUp, poiMesh.normals[float(1)])) * float(0.433) + 0.5;
        #if defined(PROP_MATCAP) || !defined(OPTIMIZER_ENABLED)
            matcap = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap, _MainTex, TRANSFORM_TEX(matcapUV, _Matcap)) * float4(1,1,1,1);
        #else
            matcap = float4(1,1,1,1);
        #endif
        matcap.rgb *= float(0.4);
        #if defined(PROP_MATCAPMASK) || !defined(OPTIMIZER_ENABLED)
        ...
        blendMatcap(finalColor, float(1), float(0), float(0), matcap, matcapMask, matcapEmission, float(0)
        #ifdef POI_LIGHTING
        , float(0)
        #endif
        #ifdef POI_BLACKLIGHT
        , _BlackLightMaskMatcap
            ...
            blendMatcap(finalColor, float(0.546), float(0.121), float(0.139), matcap2, matcap2Mask, matcapEmission, float(0)
            #ifdef POI_LIGHTING
            , float(0)
            #endif
            #ifdef POI_BLACKLIGHT
