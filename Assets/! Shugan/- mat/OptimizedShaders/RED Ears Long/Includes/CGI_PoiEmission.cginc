...
{
    float3 emission0 = 0;
    float emissionStrength0 = float(0.003);
    ...
    float glowInTheDarkMultiplier0 = calculateGlowInTheDark(float(0), float(1), float(1), float(0), float(0), float(0));
        ...
        {
            emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(3.42)).rgb * lerp(1, baseColor, float(0)).rgb * float4(1,1,1,1).rgb;
        }
    #else
    ...
    if (float(0))
    {
        emissionStrength0 *= calculateBlinkingEmission(float(0.5), float(1), float(1.42), float(0));
    }
