...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    float rimWidth = lerp( - .05, 1, float(0.726));
    #ifdef POI_AUDIOLINK
    ...
    #endif
    #if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
        rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * float4(1,0.6747245,0.5677012,1);
    #else
        rimColor = float4(1,0.6747245,0.5677012,1);
    #endif
    ...
    rim *= float4(1,0.6747245,0.5677012,1).a * rimColor.a * rimMask;
    rimLightEmission = rim * lerp(albedo, rimColor, float(0.598)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, float(0.598)) + lerp(albedo.rgb, rimColor, float(0.598)) * rimBrighten, rim);
}
#endif
