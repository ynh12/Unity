    ...
    {
        float enviroRimAlpha = saturate(1 - smoothstep(min(float(0), float(0.937)), float(0.937), poiCam.viewDotNormal));
        float(0.677) *= 1.7 - 0.7 * float(0.677);
        float3 enviroRimColor = 0;
        float interpolator = unity_SpecCube0_BoxMin.w;
        
        if (interpolator < 0.99999)
        {
            float4 reflectionData0 = UNITY_SAMPLE_TEXCUBE_LOD(unity_SpecCube0, poiMesh.normals[1], float(0.677) * UNITY_SPECCUBE_LOD_STEPS);
            float3 reflectionColor0 = DecodeHDR(reflectionData0, unity_SpecCube0_HDR);
            float4 reflectionData1 = UNITY_SAMPLE_TEXCUBE_SAMPLER_LOD(unity_SpecCube1, unity_SpecCube0, poiMesh.normals[1], float(0.677) * UNITY_SPECCUBE_LOD_STEPS);
        ...
        {
            float4 reflectionData = UNITY_SAMPLE_TEXCUBE_LOD(unity_SpecCube0, poiMesh.normals[1], float(0.677) * UNITY_SPECCUBE_LOD_STEPS);
            ...
            half enviroMask = 1;
        #endif
        return lerp(0, max(0, (enviroRimColor - float(0)) * albedo.rgb), enviroRimAlpha).rgb * enviroMask * float(0.111);
    }
#endif
