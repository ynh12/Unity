...
void applyDecals(inout float4 albedo, inout float3 decalEmission)
{
    ...
    decalScale = float4(1,1,0,0);
        ...
        decalColor = POI2D_SAMPLER_PAN(_DecalTexture, _MainTex, uv, float4(0,0,0,0)) * float4(0.7433497,0.5066087,0.4710504,1);
    #else
        uv = decalUV(float(0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float(0));
        decalColor = float4(0.7433497,0.5066087,0.4710504,1);
    #endif
    ...
    #endif
    albedo.rgb = lerp(albedo.rgb, customBlend(albedo.rgb, decalColor.rgb, float(2)), decalColor.a * saturate(_DecalBlendAlpha + audioLinkDecalAlpha0));
    ...
    decalEmission += decalColor.rgb * decalColor.a * max(float(0) + audioLinkDecalEmission0, 0);
        ...
        decalRotation = 0;
            ...
            #endif
            uv = decalUV(float(0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float(0));
            decalColor = POI2D_SAMPLER_PAN(_DecalTexture1, _MainTex, uv, float4(0,0,0,0)) * float4(0.673178,0.5921457,0.5708349,1);
        #else
            uv = decalUV(float(0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float(0));
            decalColor = float4(0.673178,0.5921457,0.5708349,1);
        #endif
        decalColor.rgb = decalHueShift(float(0), decalColor.rgb, float(0), float(0));
        ...
        #endif
        albedo.rgb = lerp(albedo.rgb, customBlend(albedo.rgb, decalColor.rgb, float(10)), decalColor.a * saturate(_DecalBlendAlpha1 + audioLinkDecalAlpha1));
        ...
        #endif
        albedo.rgb = lerp(albedo.rgb, customBlend(albedo.rgb, decalColor.rgb, float(0)), decalColor.a * saturate(_DecalBlendAlpha2 + audioLinkDecalAlpha2));
