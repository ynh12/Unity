...
float3 calculateSpecular(in float4 albedo)
{
    ...
    if (float(1) == 4)
    {
        ...
        if (float(1) == 2) // Toon
        {
            finalSpecular += calculateToonSpecular(albedo, poiMesh.uv[0], float4(0,0.5615933,0,1), metallic, float(1), specularMap, specularLightColor, poiMesh.normals[float(1)], poiLight.halfDir, poiLight.attenuation);
