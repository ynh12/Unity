    ...
    {
        #if defined(PROP_VERTEXMANIPULATIONHEIGHTMASK) || !defined(OPTIMIZER_ENABLED)
            float3 heightOffset = (tex2Dlod(_VertexManipulationHeightMask, float4(TRANSFORM_TEX(uv, _VertexManipulationHeightMask) + float4(0,2,0,0) * _Time.x, 0, 0)).r - float(0)) * float(0.04) * worldNormal;
        #else
            float3 heightOffset = float(0.04) * worldNormal;
        #endif
        worldPos.rgb += float4(0,-0.04,0.02,1).xyz/* * float4(0,-0.04,0.02,1).w*/ + heightOffset;
