    ...
    {
        #if defined(PROP_VERTEXMANIPULATIONHEIGHTMASK) || !defined(OPTIMIZER_ENABLED)
            float3 heightOffset = (tex2Dlod(_VertexManipulationHeightMask, float4(TRANSFORM_TEX(uv, _VertexManipulationHeightMask) + float4(0.49,4.15,0,0) * _Time.x, 0, 0)).r - float(0)) * float(0.0011) * worldNormal;
        #else
            float3 heightOffset = float(0.0011) * worldNormal;
        #endif
