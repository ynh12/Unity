...
void applyDecals(inout float4 albedo, inout float3 decalEmission)
{
    ...
    decalScale = float4(1,1,0,0);
        ...
        #endif
        uv = decalUV(float(2), float4(0.5,0.5,0,0), float(8) + decalRotation, float(0), decalScale, float(0));
        decalColor = POI2D_SAMPLER_PAN(_DecalTexture, _MainTex, uv, float4(0,7.72,0,0)) * float4(1,1,1,1);
    #else
        uv = decalUV(float(2), float4(0.5,0.5,0,0), float(8) + decalRotation, float(0), decalScale, float(0));
    ...
    #endif
    albedo.rgb = lerp(albedo.rgb, customBlend(albedo.rgb, decalColor.rgb, float(0)), decalColor.a * saturate(float(0.156) + audioLinkDecalAlpha0));
    ...
    decalEmission += decalColor.rgb * decalColor.a * max(float(0) + audioLinkDecalEmission0, 0);
        ...
        decalRotation = 0;
            ...
            decalColor = POI2D_SAMPLER_PAN(_DecalTexture1, _MainTex, uv, float4(0,1.9,0,0)) * float4(1,1,1,1);
        #else
            uv = decalUV(float(0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float(0));
            decalColor = float4(1,1,1,1);
        #endif
        ...
        #endif
        albedo.rgb = lerp(albedo.rgb, customBlend(albedo.rgb, decalColor.rgb, float(9)), decalColor.a * saturate(float(1) + audioLinkDecalAlpha1));
        ...
        decalRotation = 0;
            ...
            #endif
            uv = decalUV(float(0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float(0));
            decalColor = POI2D_SAMPLER_PAN(_DecalTexture2, _MainTex, uv, float4(0,0,0,0)) * float4(1,0,0,1);
        #else
            uv = decalUV(float(0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float(0));
            decalColor = float4(1,0,0,1);
        #endif
        decalColor.rgb = decalHueShift(float(1), decalColor.rgb, _DecalHueShift2, float(0));
        ...
        #endif
        albedo.rgb = lerp(albedo.rgb, customBlend(albedo.rgb, decalColor.rgb, float(0)), decalColor.a * saturate(float(0) + audioLinkDecalAlpha2));
        ...
        #endif
        albedo.rgb = lerp(albedo.rgb, customBlend(albedo.rgb, decalColor.rgb, float(6)), decalColor.a * saturate(float(1) + audioLinkDecalAlpha3));
