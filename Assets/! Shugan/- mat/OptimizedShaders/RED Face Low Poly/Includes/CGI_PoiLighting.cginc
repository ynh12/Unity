...
UnityIndirect CreateIndirectLight(float3 normal)
{
    ...
    indirectLight.specular = 0;
        ...
        if (float(1))
        {
            occlusion = lerp(1, POI2D_SAMPLER_PAN(_LightingAOTex, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)).r, float(0.162));
        }
...
void calculateBasePassLightMaps()
{
        ...
        shadowStrength = 1;
        #ifndef OUTLINE
            shadowStrength = POI2D_SAMPLER_PAN(_LightingShadowMask, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * float(0.46);
        #else
        ...
        switch(float(2))
        {
            ...
            case 1: // Math Gradient
            {
                ...
                if (float(0))
                {
                    poiLight.finalLighting = lerp((directLighting * shadowColor * poiLight.occlusion), (directLighting), saturate(poiLight.rampedLightMap + 1 - float(0.46)));
                }
                else
                {
                    poiLight.finalLighting = lerp((indirectLighting * shadowColor * poiLight.occlusion), (directLighting), saturate(poiLight.rampedLightMap + 1 - float(0.46)));
                }
            }
            break;
            case 2:
        ...
        if (float(0) == 2) // Wrapped
        {
            ...
            uint normalsindex = float(0) > 0 ? 1: 0;
                    ...
                    if (float(2) == 0) // Ramp Texture
                    {
                        poiLight.rampedLightMap = lerp(1, UNITY_SAMPLE_TEX2D(_ToonRamp, poiLight.lightMap + float(0)).rgb, shadowStrength.r);
                    }
                    else if (float(2) == 1) // Math Gradient
                    {
                    ...
                    poiLight.finalLighting = (poiLight.indirectLighting + poiLight.directLighting) * saturate(poiLight.rampedLightMap + 1 - float(0.46));
                }
                if (!float(0))
                {
        ...
        void applyShadeMaps(inout float4 albedo)
        {
            ...
            if (float(2) == 2)
            {
                ...
                float MainColorFeatherStep = float(0.125) - float(0.581);
                ...
                #endif
                firstShadeMap = lerp(firstShadeMap, albedo, float(1));
                ...
                firstShadeMap.rgb *= float4(0.7615293,0.5326606,0.5076728,1).rgb; //* lighColor
                ...
                float mainShadowMask = saturate(1 - ((poiLight.lightMap) - MainColorFeatherStep) / (float(0.125) - MainColorFeatherStep) * (shadowMask));
