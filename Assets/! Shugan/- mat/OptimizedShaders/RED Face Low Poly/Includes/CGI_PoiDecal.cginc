...
void applyDecals(inout float4 albedo, inout float3 decalEmission)
{
    ...
    #endif
    albedo.rgb = lerp(albedo.rgb, customBlend(albedo.rgb, decalColor.rgb, float(2)), decalColor.a * saturate(float(0.344) + audioLinkDecalAlpha0));
    ...
    decalEmission += decalColor.rgb * decalColor.a * max(float(0) + audioLinkDecalEmission0, 0);
        ...
        #endif
        albedo.rgb = lerp(albedo.rgb, customBlend(albedo.rgb, decalColor.rgb, float(2)), decalColor.a * saturate(_DecalBlendAlpha1 + audioLinkDecalAlpha1));
        ...
        decalRotation = 0;
            ...
            #endif
            uv = decalUV(float(2), float4(0.5,0.55,0,0), float(0) + decalRotation, float(0), decalScale, float(0));
            decalColor = POI2D_SAMPLER_PAN(_DecalTexture3, _MainTex, uv, float4(0,0,0,0)) * float4(1,1,1,1);
        #else
            uv = decalUV(float(2), float4(0.5,0.55,0,0), float(0) + decalRotation, float(0), decalScale, float(0));
            decalColor = float4(1,1,1,1);
        #endif
        decalColor.rgb = decalHueShift(float(1), decalColor.rgb, _DecalHueShift3, float(0));
        ...
        #endif
        decalEmission += decalColor.rgb * decalColor.a * max(float(0.2) + audioLinkDecalEmission3, 0);
    #endif
