...
{
    float3 emission0 = 0;
    float emissionStrength0 = float(0.67);
    ...
    float3 emissionColor1 = 0;
        ...
        float glowInTheDarkMultiplier1 = calculateGlowInTheDark(float(0), float(1), float(1), float(0), float(0), float(0));
            ...
            if (!float(0))
            {
                emissionColor1 = POI2D_SAMPLER_PAN(_EmissionMap1, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * lerp(1, baseColor, float(0)).rgb * float4(0.6039216,0.3381898,0.1686274,1).rgb;
            }
            else
            {
                emissionColor1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap1, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, baseColor, float(0)).rgb * float4(0.6039216,0.3381898,0.1686274,1).rgb;
            }
        #else
            emissionColor1 = lerp(1, baseColor, float(0)).rgb * float4(0.6039216,0.3381898,0.1686274,1).rgb;;
        #endif
