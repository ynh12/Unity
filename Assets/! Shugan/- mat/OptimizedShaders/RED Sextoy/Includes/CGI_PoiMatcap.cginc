    ...
    void applyMatcap(inout float4 finalColor, inout float3 matcapEmission)
    {
        ...
        if(float(1))
        {
            matcap.rgb = hueShift(matcap.rgb, float(0.616) + _Time.x * float(0));
        }
        blendMatcap(finalColor, float(1), float(1), float(1), matcap, matcapMask, matcapEmission, float(0)
        #ifdef POI_LIGHTING
        , float(0)
        #endif
        #ifdef POI_BLACKLIGHT
        , float(4)
            ...
            if(float(1))
            {
                matcap2.rgb = hueShift(matcap2.rgb, float(0.421) + _Time.x * float(0));
            }
            blendMatcap(finalColor, float(1), float(0), float(0), matcap2, matcap2Mask, matcapEmission, float(0)
            #ifdef POI_LIGHTING
            , float(0)
            #endif
            #ifdef POI_BLACKLIGHT
