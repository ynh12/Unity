    ...
    #endif
    {
        ...
        if(float(1))
        {
			...
			float behind = smoothstep(-float(0.3078522)*0.5, float(0.3078522)*0.2, orificePositionTracker.z);
			orificePositionTracker.z=(abs(orificePositionTracker.z+(float(0.3078522)*0.2))-(float(0.3078522)*0.2))*(1+step(orificePositionTracker.z,0)*2);
			orificePositionTracker.z=smoothstep(-float(0.3078522)*0.2, float(0.3078522)*0.2, orificePositionTracker.z) * orificePositionTracker.z;
			float distanceToOrifice = length( orificePositionTracker );
			float3 PhysicsNormal = normalize(penetratorNormalTracker.xyz);
			float enterFactor = smoothstep( float(0.3078522) , float(0.3078522)+0.05 , distanceToOrifice);
			float wriggleTimeY = _Time.y * float(0.28);
			float curvatureMod = ( float(0.3078522) * ( ( cos( wriggleTimeY ) * float(0.04) ) + float(0) ) );
			float wriggleTimeX = _Time.y * ( float(0.28) * 0.79 );
			float3 finalOrificeNormal = normalize( lerp( orificeNormal , ( PhysicsNormal + ( ( float3(0,1,0) * ( curvatureMod + ( float(0.3078522) * ( float(0) + ( ( sin( wriggleTimeY ) * 0.3 ) * float(0.04) ) ) * 2.0 ) ) ) + ( float3(0.5,0,0) * ( cos( wriggleTimeX ) * float(0.04) ) ) ) ) , enterFactor) );
			float3 finalOrificePosition = lerp( orificePositionTracker , ( ( normalize(penetratorNormalTracker) * float(0.3078522) ) + ( float3(0,0.2,0) * ( sin( ( wriggleTimeY + UNITY_PI ) ) * float(0.04) ) * float(0.3078522) ) + ( float3(0.2,0,0) * float(0.3078522) * ( sin( ( wriggleTimeX + UNITY_PI ) ) * float(0.04) ) ) ) , enterFactor);
			...
			float3 curvatureOffset = lerp( float3( 0,0,0 ) , ( float3(0,1,0) * ( curvatureMod * -0.2 ) ) , saturate( ( distanceToOrifice / float(0.3078522) ) ));
