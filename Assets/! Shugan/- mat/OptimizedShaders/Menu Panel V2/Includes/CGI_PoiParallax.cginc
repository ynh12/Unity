    ...
    float2 ParallaxRaymarching(float2 viewDir)
    {
        float2 uvOffset = 0;
        float stepSize = 0.1;
        float2 uvDelta = viewDir * (stepSize * float(0.004));
        float stepHeight = 1;
    ...
    void calculateAndApplyInternalParallax(inout float4 finalColor)
    {
            ...
            {
                float3 parallax = 0;
                for (int j = float(2.8); j > 0; j --)
                {
                    float ratio = (float)j / float(2.8);
                    float2 parallaxOffset = _Time.y * (float4(0,0,0,0) + (1 - ratio) * float4(0,0,0,0));
                    float fade = lerp(float(0.4), float(0.11), ratio);
                    #if defined(PROP_PARALLAXINTERNALMAP) || !defined(OPTIMIZER_ENABLED)
                        float4 parallaxColor = UNITY_SAMPLE_TEX2D_SAMPLER(_ParallaxInternalMap, _MainTex, TRANSFORM_TEX(poiMesh.uv[0], _ParallaxInternalMap) + lerp(float(-0.08), float(0.16), ratio) * - poiCam.tangentViewDir.xy + parallaxOffset);
                    #else
