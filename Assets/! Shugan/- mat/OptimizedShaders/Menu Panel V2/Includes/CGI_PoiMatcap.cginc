    ...
    void applyMatcap(inout float4 finalColor, inout float3 matcapEmission)
    {
        ...
        #if defined(PROP_MATCAP) || !defined(OPTIMIZER_ENABLED)
            matcap = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap, _MainTex, TRANSFORM_TEX(matcapUV, _Matcap)) * float4(0.5152035,0.03778511,0.03778511,1);
        #else
            matcap = float4(0.5152035,0.03778511,0.03778511,1);
        #endif
        matcap.rgb *= float(1);
        #if defined(PROP_MATCAPMASK) || !defined(OPTIMIZER_ENABLED)
            matcapMask = POI2D_SAMPLER_PAN(_MatcapMask, _MainTex, poiMesh.uv[float(2)], float4(0,0,0,0));
        #else
            matcapMask = 1;
        #endif
        if (float(0))
        ...
        blendMatcap(finalColor, float(0), float(0), float(1), matcap, matcapMask, matcapEmission, float(1)
        #ifdef POI_LIGHTING
        , float(0)
        #endif
        #ifdef POI_BLACKLIGHT
