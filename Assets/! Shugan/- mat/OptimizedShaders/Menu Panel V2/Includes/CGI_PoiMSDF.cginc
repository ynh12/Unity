    ...
    void ApplyPositionText(inout float4 albedo, float2 uv)
    {
        ...
        uv = TransformUV(float4(0,0,0,0), float(0), float4(1,0.1,1,1), uv);
        ...
        globalTextEmission += float4(1,0,1,1).rgb * opacity * float(0.75);
    }
    void ApplyTimeText(inout float4 albedo, float2 uv)
    {
        ...
        uv = TransformUV(float4(0.4,0.46,0,0), float(0), float4(0.16,0.03,1,1), uv);
        if(uv.x > 1 || uv.x < 0 || uv.y > 1 || uv.y < 0)
        {
            return;
        }
        ...
        sigDist *= max(dot(msdfUnit, 0.5 / fwidth(uv)), 1);
        float opacity = clamp(sigDist + 0.5, 0, 1);
        albedo.rgb = lerp(albedo.rgb, float4(0,0.3259627,1,1).rgb, opacity * float4(0,0.3259627,1,1).a);
        globalTextEmission += float4(0,0.3259627,1,1).rgb * opacity * float(2.2);
    }
    void ApplyFPSText(inout float4 albedo, float2 uv)
    {
        ...
        uv = TransformUV(float4(0.4,0.43,0,0), float(0), float4(0.16,0.03,1,1), uv);
        if(uv.x > 1 || uv.x < 0 || uv.y > 1 || uv.y < 0)
        {
            return;
        }
        ...
        sigDist *= max(dot(msdfUnit, 0.5 / fwidth(uv)), 1);
        float opacity = clamp(sigDist + 0.5, 0, 1);
        albedo.rgb = lerp(albedo.rgb, float4(0,0.3277781,1,1).rgb, opacity * float4(0,0.3277781,1,1).a);
        globalTextEmission += float4(0,0.3277781,1,1).rgb * opacity * float(1.59);
    }
