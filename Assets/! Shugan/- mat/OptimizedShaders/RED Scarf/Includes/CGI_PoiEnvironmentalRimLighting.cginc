    ...
    {
        float enviroRimAlpha = saturate(1 - smoothstep(min(float(0), float(1)), float(1), poiCam.viewDotNormal));
        ...
        else
        {
            ...
            half enviroMask = 1;
        #endif
        return lerp(0, max(0, (enviroRimColor - float(0)) * albedo.rgb), enviroRimAlpha).rgb * enviroMask * float(0.066);
    }
#endif
