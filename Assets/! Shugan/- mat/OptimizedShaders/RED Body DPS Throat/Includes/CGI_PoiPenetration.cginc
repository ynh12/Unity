    ...
    #endif
    {
        ...
        if(float(0))
        {
			...
			float behind = smoothstep(-float(0)*0.5, float(0)*0.2, orificePositionTracker.z);
			orificePositionTracker.z=(abs(orificePositionTracker.z+(float(0)*0.2))-(float(0)*0.2))*(1+step(orificePositionTracker.z,0)*2);
			orificePositionTracker.z=smoothstep(-float(0)*0.2, float(0)*0.2, orificePositionTracker.z) * orificePositionTracker.z;
			float distanceToOrifice = length( orificePositionTracker );
			float3 PhysicsNormal = normalize(penetratorNormalTracker.xyz);
			float enterFactor = smoothstep( float(0) , float(0)+0.05 , distanceToOrifice);
			float wriggleTimeY = _Time.y * float(0.28);
			float curvatureMod = ( float(0) * ( ( cos( wriggleTimeY ) * float(0) ) + float(0) ) );
			float wriggleTimeX = _Time.y * ( float(0.28) * 0.79 );
			float3 finalOrificeNormal = normalize( lerp( orificeNormal , ( PhysicsNormal + ( ( float3(0,1,0) * ( curvatureMod + ( float(0) * ( float(0) + ( ( sin( wriggleTimeY ) * 0.3 ) * float(0) ) ) * 2.0 ) ) ) + ( float3(0.5,0,0) * ( cos( wriggleTimeX ) * float(0) ) ) ) ) , enterFactor) );
			float3 finalOrificePosition = lerp( orificePositionTracker , ( ( normalize(penetratorNormalTracker) * float(0) ) + ( float3(0,0.2,0) * ( sin( ( wriggleTimeY + UNITY_PI ) ) * float(0) ) * float(0) ) + ( float3(0.2,0,0) * float(0) * ( sin( ( wriggleTimeX + UNITY_PI ) ) * float(0) ) ) ) , enterFactor);
			...
			float3 curvatureOffset = lerp( float3( 0,0,0 ) , ( float3(0,1,0) * ( curvatureMod * -0.2 ) ) , saturate( ( distanceToOrifice / float(0) ) ));
			...
			VertexPosition.xyz = lerp( VertexPosition.xyz , ( float3(0,0, distanceToOrifice) + ( min( length( sphereifyDistance ) , float(0) ) * sphereifyNormal ) ) , sphereifyFactor);
			float squeezeFactor = smoothstep( 0.0 , float(0) , VertexPosition.z - distanceToOrifice);
			squeezeFactor = max( squeezeFactor , smoothstep( 0.0 , float(0) , distanceToOrifice - VertexPosition.z));
			squeezeFactor = 1- (1-squeezeFactor) * smoothstep(0,0.01,VertexPosition.z) * behind * (1-enterFactor);
			VertexPosition.xy = lerp( ( normalize(VertexPosition.xy) * min( length( VertexPosition.xy ) , float(0) ) ) , VertexPosition.xy , squeezeFactor);
			float bulgeFactor = 1-smoothstep( 0.0 , float(0) , abs( ( finalOrificeDistance - VertexPosition.z ) ));
			float bulgeFactorBaseClip = smoothstep( 0.0 , 0.05 , VertexPosition.z);
			VertexPosition.xy *= lerp( 1.0 , ( 1.0 + float(0) ) , ( bulgeFactor * bulgeFactorBaseClip * behind * (1-enterFactor)));
    ...
    float3 getBlendOffset(float blendSampleIndex, float activationDepth, float activationSmooth, int vertexID, float penetrationDepth, float3 normal, float3 tangent, float3 binormal)
    {
        ...
        float3 blendOffset = ((sampledBlend - float3(1, 1, 1)) * (blendActivation) * float(0.1) * float(1));
    ...
    #endif
    {
        ...
        if (float(1))
        {
            ...
            v.vertex.xyz += getBlendOffset(0, 0, float(0.042), v.vertexId, penetrationDepth, normal, tangent, binormal);
            ...
            v.vertex.w = 1;
            v.normal += getBlendOffset(1, 0, float(0.042), v.vertexId, penetrationDepth, normal, tangent, binormal);
