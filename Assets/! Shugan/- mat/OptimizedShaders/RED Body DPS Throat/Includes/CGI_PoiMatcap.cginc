    ...
    void applyMatcap(inout float4 finalColor, inout float3 matcapEmission)
    {
        ...
        half2 matcapUV = half2(dot(worldViewRight, poiMesh.normals[float(0)]), dot(worldViewUp, poiMesh.normals[float(0)])) * float(0.43) + 0.5;
        #if defined(PROP_MATCAP) || !defined(OPTIMIZER_ENABLED)
            matcap = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap, _MainTex, TRANSFORM_TEX(matcapUV, _Matcap)) * float4(1,1,1,1);
        #else
            matcap = float4(1,1,1,1);
        #endif
        matcap.rgb *= float(0.3);
        #if defined(PROP_MATCAPMASK) || !defined(OPTIMIZER_ENABLED)
        ...
        blendMatcap(finalColor, float(0.265), float(0.01), float(0), matcap, matcapMask, matcapEmission, float(0)
        #ifdef POI_LIGHTING
        , float(0)
        #endif
        #ifdef POI_BLACKLIGHT
        , float(4)
            ...
            #if defined(PROP_MATCAP2) || !defined(OPTIMIZER_ENABLED)
                matcap2 = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap2, _MainTex, TRANSFORM_TEX(matcapUV2, _Matcap2)) * float4(1,0.06295128,0.06295128,1);
            #else
                matcap2 = float4(1,0.06295128,0.06295128,1);
            #endif
            matcap2.rgb *= float(1);
            #if defined(PROP_MATCAP2MASK) || !defined(OPTIMIZER_ENABLED)
            ...
            blendMatcap(finalColor, float(0.035), float(0.149), float(0.138), matcap2, matcap2Mask, matcapEmission, float(0)
            #ifdef POI_LIGHTING
            , float(0)
            #endif
            #ifdef POI_BLACKLIGHT
