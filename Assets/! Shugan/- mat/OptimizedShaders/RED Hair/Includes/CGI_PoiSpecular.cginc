...
float3 calculateSpecular(in float4 albedo)
{
    ...
    if (float(2) == 1) // Realistic
    {
        ...
        finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(0.03864443,0.03864443,0.03864443,1), metallic, float(1) * specularMap.a, poiLight.dotNH, poiLight.dotLH, specularLightColor, attenuation);
    }
    ...
    if (float(2) == 4)
    {
        ...
        finalSpecular += toonAnisoSpecular(float(0), float(0), poiLight.direction, poiLight.halfDir, specularMap, poiLight.nDotL, float(0.95), float(1), float4(0.03864443,0.03864443,0.03864443,1), albedo, metallic, jitter, float(0), packedTangentMap);
        ...
        if (float(2) == 2) // Toon
        {
            ...
            finalSpecular *= float4(0.03864443,0.03864443,0.03864443,1);
        }
        ...
        if (float(2) == 3) // anisotropic
        {
            ...
            jitter += float(0);
            float4 packedTangentMap = POI2D_SAMPLER_PAN(_AnisoTangentMap, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0));
            finalSpecular += AnisotropicSpecular(float(0), float(0), float(1), float(0), float(1), float(1), float4(0.03864443,0.03864443,0.03864443,1), metallic, specularMap, specularLightColor, poiLight.direction, poiLight.halfDir, poiLight.nDotL, jitter, packedTangentMap, albedo);
            finalSpecular *= float4(0.03864443,0.03864443,0.03864443,1);
            finalSpecular *= attenuation;
        }
    #endif
        ...
        for (int index = 0; index < 4; index++)
        {
            ...
            if (float(2) == 1) // Realistic
            {
                finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(0.03864443,0.03864443,0.03864443,1), metallic, float(1) * specularMap.a, poiLight.vDotNH[index], poiLight.vDotLH[index], poiLight.vColor[index], poiLight.vAttenuationDotNL[index]);
            }
        }
    #endif
    finalSpecular *= float4(0.03864443,0.03864443,0.03864443,1).a;
    ...
    finalSpecular *= specularMask;
        ...
        if (float(1) == 1) // Realistic
        {
            ...
            finalSpecular1 = calculateNewSpecular(specularMap1.rgb, float(1), realisticAlbedo1, float4(0.09441278,0.09441278,0.09441278,1), metallic1, float(0.68) * specularMap1.a, poiLight.dotNH, poiLight.dotLH, specularLightColor, attenuation);
        }
        ...
        if (float(1) == 4)
        {
            ...
            finalSpecular1 += toonAnisoSpecular(float(0), float(0), poiLight.direction, poiLight.halfDir, specularMap1, poiLight.nDotL, float(0.95), float(1), float4(0.09441278,0.09441278,0.09441278,1), albedo, metallic1, jitter, float(0), packedTangentMap);
        ...
        if (float(1) == 2) // Toon
        {
            ...
            finalSpecular1 *= float4(0.09441278,0.09441278,0.09441278,1);
        }
        ...
        if (float(1) == 3) // anisotropic
        {
            ...
            jitter += float(0);
            float4 packedTangentMap = POI2D_SAMPLER_PAN(_AnisoTangentMap1, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0));
            finalSpecular1 = AnisotropicSpecular(float(0), float(0), float(0.68), float(0), float(1), float(1), float4(0.09441278,0.09441278,0.09441278,1), metallic1, specularMap1, specularLightColor, poiLight.direction, poiLight.halfDir, poiLight.nDotL, jitter, packedTangentMap, albedo);
            finalSpecular1 *= float4(0.09441278,0.09441278,0.09441278,1);
            finalSpecular1 *= poiLight.attenuation;
        }
        #ifdef FORWARD_BASE_PASS
                ...
                for (int index = 0; index < 4; index++)
                {
                    ...
                    if (float(2) == 1) // Realistic
                    {
                        finalSpecular1 += calculateNewSpecular(specularMap1.rgb, float(1), realisticAlbedo1, float4(0.09441278,0.09441278,0.09441278,1), metallic1, float(0.68) * specularMap1.a, poiLight.vDotNH[index], poiLight.vDotLH[index], poiLight.vColor[index], poiLight.vAttenuationDotNL[index]);
                    }
                }
            #endif
        #endif
        finalSpecular1 *= float4(0.09441278,0.09441278,0.09441278,1).a;
