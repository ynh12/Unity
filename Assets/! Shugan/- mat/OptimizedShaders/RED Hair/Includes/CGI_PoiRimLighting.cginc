...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    float rimStrength = float(0.003);
    float rimBrighten = float(0.5);
    float rimWidth = lerp( - .05, 1, float(0.595));
    #ifdef POI_AUDIOLINK
    ...
    rimLightEmission = rim * lerp(albedo, rimColor, float(0.084)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, float(0.084)) + lerp(albedo.rgb, rimColor, float(0.084)) * rimBrighten, rim);
}
#endif
