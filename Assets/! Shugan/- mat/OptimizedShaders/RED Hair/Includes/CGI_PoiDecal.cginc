...
void applyDecals(inout float4 albedo, inout float3 decalEmission)
{
    ...
    decalScale = float4(1,1,0,0);
        ...
        decalColor = POI2D_SAMPLER_PAN(_DecalTexture, _MainTex, uv, float4(0,0,0,0)) * float4(1,0.3398705,0.3398705,1);
    #else
        uv = decalUV(float(0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float(0));
        decalColor = float4(1,0.3398705,0.3398705,1);
    #endif
    decalColor.rgb = decalHueShift(float(1), decalColor.rgb, _DecalHueShift, float(0));
    ...
    #endif
    albedo.rgb = lerp(albedo.rgb, customBlend(albedo.rgb, decalColor.rgb, float(6)), decalColor.a * saturate(float(0.156) + audioLinkDecalAlpha0));
    ...
    #endif
    decalEmission += decalColor.rgb * decalColor.a * max(float(0.003) + audioLinkDecalEmission0, 0);
