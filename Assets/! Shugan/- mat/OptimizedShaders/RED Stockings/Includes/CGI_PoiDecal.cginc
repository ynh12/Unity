...
void applyDecals(inout float4 albedo, inout float3 decalEmission)
{
    ...
    #endif
    albedo.rgb = lerp(albedo.rgb, customBlend(albedo.rgb, decalColor.rgb, float(1)), decalColor.a * saturate(_DecalBlendAlpha + audioLinkDecalAlpha0));
    ...
    decalEmission += decalColor.rgb * decalColor.a * max(float(0) + audioLinkDecalEmission0, 0);
        ...
        #endif
        albedo.rgb = lerp(albedo.rgb, customBlend(albedo.rgb, decalColor.rgb, float(1)), decalColor.a * saturate(_DecalBlendAlpha1 + audioLinkDecalAlpha1));
