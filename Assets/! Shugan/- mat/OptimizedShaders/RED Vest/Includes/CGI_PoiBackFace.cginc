...
void applyBackFaceTexture(inout float backFaceDetailIntensity, inout float mixedHueShift, inout float4 albedo, inout float3 backFaceEmission)
{
	...
	if (float(1))
	{
		...
		{
			#if defined(PROP_BACKFACETEXTURE) || !defined(OPTIMIZER_ENABLED)
				float4 backFaceTex = POI2D_SAMPLER_PAN(_BackFaceTexture, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * float4(0.0651107,0.001963227,0.001963227,1);
			#else
				float4 backFaceTex = float4(0.0651107,0.001963227,0.001963227,1);
			#endif
