...
void applyDecals(inout float4 albedo, inout float3 decalEmission)
{
    ...
    decalEmission += decalColor.rgb * decalColor.a * max(float(0) + audioLinkDecalEmission0, 0);
        ...
        decalRotation = 0;
            ...
            #endif
            uv = decalUV(float(0), float4(0.5,0.492,0,0), float(0) + decalRotation, float(0), decalScale, float(0));
        ...
        #endif
        albedo.rgb = lerp(albedo.rgb, customBlend(albedo.rgb, decalColor.rgb, float(0)), decalColor.a * saturate(float(0.5) + audioLinkDecalAlpha2));
