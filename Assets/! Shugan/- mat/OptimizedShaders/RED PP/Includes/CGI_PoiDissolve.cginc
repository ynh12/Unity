...
void calculateDissolve(inout float4 albedo, inout float3 dissolveEmission)
{
    ...
    #endif
    #if defined(PROP_DISSOLVENOISETEXTURE) || !defined(OPTIMIZER_ENABLED)
        float dissolveNoiseTexture = POI2D_SAMPLER_PAN(_DissolveNoiseTexture, _MainTex, poiMesh.uv[float(1)], float4(0,0,0,0)).r;
    #else
    ...
    if (float(1) == 1) // Basic
    {
        ...
        edgeAlpha = remapClamped(noise, da + float(0.045), da, 0, 1) * (1 - dissolveAlpha);
    }
    ...
    if (float(0.045))
    {
        edgeColor = tex2D(_DissolveEdgeGradient, TRANSFORM_TEX(float2(edgeAlpha, edgeAlpha), _DissolveEdgeGradient)) * float4(1,0.07184538,0.07184538,0);
        #ifndef POI_SHADOW
            
            if (float(0))
            {
                edgeColor.rgb = hueShift(edgeColor.rgb, _DissolveEdgeHueShift + _Time.x * float(0));
            }
        #endif
        albedo.rgb = lerp(albedo.rgb, edgeColor.rgb, smoothstep(0, 1 - float(0) * .99999999999, edgeAlpha));
    }
    dissolveEmission = lerp(0, dissolveToTexture * float(0), dissolveAlpha) + lerp(0, edgeColor.rgb * float(0), smoothstep(0, 1 - float(0) * .99999999999, edgeAlpha));
}
#endif
